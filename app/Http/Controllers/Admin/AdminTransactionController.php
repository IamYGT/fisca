        ...
        $baseQuery = Transaction::with('user')
            ->when($request->search, function ($query, $search) {
                ...
                return $query->where(function ($query) use ($search) {
                    ...
                    $query->whereRaw('LOWER(reference_id) LIKE ?', [$searchPattern])
                        ->orWhereRaw('CAST(amount AS TEXT) LIKE ?', [$searchPattern])
                        ->orWhereRaw('CAST(amount_usd AS TEXT) LIKE ?', [$searchPattern])
                        ->orWhereRaw('LOWER(bank_account) LIKE ?', [$searchPattern])
                        ->orWhereRaw('LOWER(crypto_address) LIKE ?', [$searchPattern])
                        ->orWhereRaw('LOWER(crypto_txid) LIKE ?', [$searchPattern])
        ...
        if ($oldStatus !== $validated['status']) {
            ...
            app(SlackService::class)->sendTransactionNotification(
                array_merge(
                    ...
                    ['status_changed' => true, 'old_status' => $oldStatus]
                )
            );
        }
