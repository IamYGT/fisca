...
import { ChatBubbleLeftIcon, BanknotesIcon, CurrencyDollarIcon, ClockIcon, ArrowTrendingUpIcon } from "@heroicons/react/24/outline";
...
const ActivityItem = ({
  ...
  const getActivityRoute = (activity2) => {
    switch (activity2.type) {
      case "transaction":
        return route("management.user.transactions.show", { transaction: activity2.id });
      case "withdrawal":
        return route("management.user.withdrawals.create");
      case "ticket":
        return route("management.user.tickets.show", { ticket: activity2.id });
      default:
        return "#";
    }
  };
